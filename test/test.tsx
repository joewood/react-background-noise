...
class TestBackground extends React.Component<{}, { scale?: number; offsetX?: number; offsetY?: number }> {
		...
		return (
			...
			>
				...
				>
					...
					<Panel heading="Offset" contents={`${this.state.offsetX},${this.state.offsetY}`} />
				</Background>
				<div key="scale" style={labelStyle}>
					<span style={{ flex: "0 0 100px" }}>Scale</span>
					<input
						ref={ctrl => (this.scale = ctrl)}
						onChange={x => this.setState({ scale: parseFloat(this.scale.value) })}
						type="range"
						value={scale}
						min={1}
						max={1000}
				...
				<div key="offsetx" style={labelStyle}>
					...
					<input
						...
						type="range"
						onChange={x => this.setState({ offsetX: parseFloat(this.offsetX.value) })}
						value={offsetX}
						min={0}
						max={100}
						step={1}
				...
				<div key="offsety" style={labelStyle}>
					...
					<input
						...
						onChange={x => this.setState({ offsetY: parseFloat(this.offsetY.value) })}
						type="range"
						value={offsetY}
						min={0}
						max={100}
